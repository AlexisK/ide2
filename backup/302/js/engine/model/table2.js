...
TABLE2.history = parseObj(glob('tbl2History')||'[]');
...
function eTable2(model, data) {
    ...
    self.fetchPages = function() {
        ...
        if ( TABLE2.history.length >= 2 ) {
            var mod = TABLE2.history[TABLE2.history.length-2];
    ...
    self.show = function(rdata) {
        ...
        TABLE2.history.remove(model);
        TABLE2.history.add(model);
        glob('tbl2History', parseStr(TABLE2.history));
...
TVIEW.mapper   = function(dict,  obj, key) { return dict[$AD(obj,key)]||'?'; }
