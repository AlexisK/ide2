...
new eEditorField('image', {
    type: 'ogimage',
    _buildMediaRow: f(key, node) {
        var row = cr('tr');
    ...
    build: f(self, data) {
        var node = _jO(cr('div', 'jImgForm'));
        
        node.V.table = node.cr('table');
        node.V.input = cr('input');
        ...
        node.V.table.attach(self.data._buildMediaRow('change', node.V.uploadBtn));
        node.V.table.attach(self.data._buildMediaRow(node.V.blockSize, node.V.strName));
        ...
        mapO(data.file, function(dims, tag) {
            ...
            node.V.table.attach(self.data._buildMediaRow(imgNodes[0], imgNodes[1]));
                ...
                readerGlob.ref.getImageDimetionsCallback(img, f(dims) {
                    ...
                    if ( file ) {
                        if ( node.C.media_id ) {
                            ...
                            ORM.prep('mediatype_'+(data.type || self.data.type), function(mediatype) {
        ...
        $P(node, 'val', function() { return node.C.media_id; }, function(val) {
            if ( val ) {
                node.C.media_id = val;
                node.F.refreshImages();
            }
