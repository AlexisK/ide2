...
new eProcessor('swipeArea', {
    ...
    _process: function(self, db) {
        ...
        self.btnLeft  = S('.mk_swipeBtnLeft' , self);
        self.btnRight = S('.mk_swipeBtnRight', self);
        ...
        map(self.btnLeft , f(btn){
            clearEvents(btn).onclick = f() {
                self._curPos[0] += self.container.offsetWidth;
                self._normalizePos();
            }
        });
        map(self.btnRight, f(btn){
            clearEvents(btn).onclick = f() {
                self._curPos[0] -= self.container.offsetWidth;
                self._normalizePos();
            }
